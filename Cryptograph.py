...
def check_aes_ecb_enc(plaintext, key):
        ...
        ciphertext = cipher.encrypt(pad(bytes(plaintext),16))
        return bytearray(ciphertext)

def check_aes_ecb_dec(ciphertext, key):
        cipher = AES.new(key, AES.MODE_ECB)
        plaintext = cipher.decrypt(pad(bytes(ciphertext),16))
        return bytearray(plaintext)

def check_des_ecb_enc(plaintext,  key):
        cipher = DES.new(key, DES.MODE_ECB)
        ciphertext = cipher.encrypt(pad(bytes(plaintext),8))
        return bytearray(ciphertext)

def check_des_ecb_dec(ciphertext, key):
        cipher = DES.new(key, DES.MODE_ECB)
        plaintext = cipher.decrypt(pad(bytes(ciphertext),8))
        return bytearray(plaintext)
